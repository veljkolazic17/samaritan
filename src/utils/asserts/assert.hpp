...
#define softAssert(condition, message, ...)                                                     \
    do {                                                                                        \
        if (condition) {                                                                        \
            ...
            show_assert(AssertType::SoftAssert, __FILE__, #condition, __LINE__, message, ##__VA_ARGS__); \
            trapBreak();                                                                        \
        }                                                                                       \
    } while(0)
...
#define hardAssert(condition, message, ...)                                                     \
    do {                                                                                        \
        if (condition) {                                                                        \
            ...
            show_assert(AssertType::HardAssert, __FILE__, #condition, __LINE__, message, ##__VA_ARGS__); \
            trapBreak();                                                                        \
            std::abort();                                                                       \
        }                                                                                       \
    } while(0)
