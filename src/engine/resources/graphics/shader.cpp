...
void Shader::OnLoad()
{
    ...
    if (f.fail())
    {
        LogError(LogChannel::Graphics, "Cannot opet shader file!");
        return false;
    }
...
void Shader::LoadShaderAttributes(void* data)
{
    ...
    const nlohmann::json attributes = nlohmann::json::parse(dataJson["attributes"]);
...
void Shader::LoadShaderUniforms(void* data)
{
    ...
    const nlohmann::json uniforms = nlohmann::json::parse(dataJson["uniforms"]);
